...
def iterative_poison_distillation(inspection_set, clean_set, clean_set_random, params, args, debug_packet=None, start_iter=0):
    ...
    if start_iter != 0:
        distilled_samples_indices, _ = confusion_training_ac.distill(args, params, inspection_set,
    ...
    freq_of_each_class = np.ones(num_classes + 1)
    ...
    for confusion_iter in range(start_iter, num_confusion_iter):
        ...
        if confusion_iter >= 4:
            ...
            confusion_training_ac.pretrain(args, debug_packet, arch, num_classes,
                                           weight_decay, pretrain_epochs,
                                           distilled_set_loader, criterion,
        ...
        model = confusion_training_ac.confusion_train(inspection_set, params, args, debug_packet, distilled_set_loader, clean_set_loader, confusion_iter, arch,
        ...
        distilled_samples_indices, median_sample_indices  = confusion_training_ac.distill(args, params, inspection_set,
    ...
    inspection_set_C = tools.IMG_Dataset(data_dir=inspection_set_img_dir,
                                       label_path=inspection_set_label_path, transforms=params['data_transform'],
                                         fixed_label=num_classes)
    num_samples = len(inspection_set_C)

    other_indices = list(set(list(range(num_samples))) - set(distilled_samples_indices))
    other_indices.sort()

    set_A = torch.utils.data.Subset(inspection_set, other_indices)
    set_B = torch.utils.data.Subset(inspection_set_C, distilled_samples_indices)
    set_C = torch.utils.data.ConcatDataset([set_A, set_B])


    C_set_loader = torch.utils.data.DataLoader(
        set_C,
        batch_size=128, shuffle=True,
        worker_init_fn=tools.worker_init, **kwargs)
        
    """confusion_training_ac.pretrain_narrow(args, debug_packet, arch, num_classes,
                                           weight_decay, pretrain_epochs,
                                           C_set_loader, criterion,
                                           inspection_set_dir, -1,
                                           0.01)"""

    confusion_training_ac.confusion_train_narrow(inspection_set, params, args, debug_packet, C_set_loader,
                                          clean_set_loader, 999, arch,
                                          num_classes, inspection_set_dir, weight_decay, criterion_no_reduction,
                                          0.5, 5,
                                          freq_of_each_class, 0.001, 2, 4000)
    ...
    model = confusion_training_ac.relabel_training(args, params, inspection_set, debug_packet, distilled_set_loader, clean_set_loader,
                                                  4, arch,
                                                  num_classes, inspection_set_dir, weight_decay, criterion_no_reduction,
                                                  0.0, 5,
                                                  freq_of_each_class, 0.001, 1,
                                                  4000)


    capture, others = confusion_training_ac.isolate(args, params, inspection_set, model, criterion_no_reduction)
    others_set = torch.utils.data.Subset(inspection_set, others)
    clean_set = torch.utils.data.ConcatDataset([clean_set, others_set])
    clean_set_loader = torch.utils.data.DataLoader(
        clean_set, batch_size=32,
        shuffle=True, worker_init_fn=tools.worker_init, **kwargs)
...
distilled_samples_indices, median_sample_indices, model = iterative_poison_distillation(inspection_set,
                                                clean_set, clean_set_random, params, args, debug_packet, start_iter=4)
